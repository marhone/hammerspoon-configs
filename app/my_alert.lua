...
local showAlert = function(message, image, style, screenObj, duration)
    ...
    if message == "" then
        ...
        textFrame = drawing.getTextDrawingSize(message, {
            font = textFont,
            size = textSize
        })
    ...
    table.insert(alertEntry.drawings,
        drawing.rectangle(drawingFrame):setStroke(true):setStrokeWidth(thisAlertStyle.strokeWidth):setStrokeColor(
            thisAlertStyle.strokeColor):setFill(true):setFillColor(thisAlertStyle.fillColor):setRoundedRectRadii(
            thisAlertStyle.radius, thisAlertStyle.radius):show(thisAlertStyle.fadeInDuration))
    ...
    if image then
        ...
        table.insert(alertEntry.drawings, drawing.image(iconFrame, image):orderAbove(alertEntry.drawings[1])
    ...
    table.insert(alertEntry.drawings,
        drawing.text(textFrame, message):setTextFont(textFont):setTextSize(textSize):setTextColor(textColor):orderAbove(
            alertEntry.drawings[1]):show(thisAlertStyle.fadeInDuration))
